...
def test_update_card(client):
    ...
    assert b'<li><strong>Q:</strong> x1' in res.data
    assert b'<li><strong>Q:</strong> x2' in res.data
    assert b'<li><strong>Q:</strong> x3' in res.data

    print(card_ids_x)
    print('Hello World!')

    # update 2nd card
    res = client.post(f'/sets/set/{set_id_x}/card/{card_ids_x[0]}', data={'front': 'xx2', 'back': 'xx2', })

    res = client.get(f'/sets/{set_id_x}')
    html = decode_html(res)
    print(html)
    assert b'<li><strong>Q:</strong> xx2' in res.data